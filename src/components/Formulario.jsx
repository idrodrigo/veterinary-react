...
const Formulario = ({ pacientes, setPacientes, paciente, setPaciente }) => {
  ...
  const labelClass = 'block text-gray-700 uppercase font-bold dark:text-white'
  const inputClass =
    'text-black border-2 w-full p-2 mt-2 rounded-lg outline-none focus:border-black dark:bg-slate-900 dark:text-gray-200 dark:border-slate-900 placeholder:text-slate-600 dark:focus:border-sky-400'
  ...
  const handleSubmit = (e) => {
    ...
    if (
      [nombrePaciente, nombreAcudiente, email, fecha, observacion].includes('')
    ) {
      setError(true)
      return
    }
    ...
    if (paciente.id) {
      ...
      const pacientesActualizados = pacientes.map((pacienteState) =>
        pacienteState.id === paciente.id ? objetoCliente : pacienteState
      )
  ...
  return (
    <div className='md:w-1/2 lg:w-2/5'>
      <h1 className='font-black text-center text-3xl text-black dark:text-sky-400'>
        Patient monitoring
      </h1>
      <p className='text-lg mt-5 text-center mb-5 text-black dark:text-gray-200'>
        Add and{' '}
      ...
      >
        ...
        <div className='mb-5'>
          <label htmlFor='acudiente' className={labelClass}>
            Owner name
          </label>
          <input
            id='acudiente'
